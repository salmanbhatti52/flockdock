    ...
    return Wrap(
      ...
      children: [
        for (int i = 0; i < groupDetail.features!.length; i++)
          InkWell(
              onTap: () {
                if (eventDetail.features!
                    .contains(groupDetail.features![i].featureId)) {
                  eventDetail.features
                      ?.remove(groupDetail.features![i].featureId);
                } else {
                  eventDetail.features
                      ?.add(groupDetail.features![i].featureId!);
                }
                setState(() {});
              },
              child: ValueContainer(
                  value: groupDetail.features![i].feature!,
                  isSelected: eventDetail.features!
                      .contains(groupDetail.features![i].featureId))),
      ],
    );
  }
